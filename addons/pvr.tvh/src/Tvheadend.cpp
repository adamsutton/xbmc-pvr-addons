...
PVR_ERROR CTvheadend::GetRecordingEdl
  ( const PVR_RECORDING &rec, PVR_EDL_ENTRY edl[], int *size )
{
  return PVR_ERROR_NO_ERROR;
}
...
PVR_ERROR CTvheadend::AddTimer ( const PVR_TIMER &tmr )
{
  return PVR_ERROR_NO_ERROR;
}

PVR_ERROR CTvheadend::DeleteTimer ( const PVR_TIMER &tmr, bool force )
{
  return PVR_ERROR_NO_ERROR;
}

PVR_ERROR CTvheadend::UpdateTimer ( const PVR_TIMER &tmr )
{
  return PVR_ERROR_NO_ERROR;
}
...
bool CTvheadend::Reconnected ( void )
{
  htsmsg_t *msg;
  STags::iterator tit;
  SChannels::iterator cit;
...
void CTvheadend::ParseRecordingDelete ( htsmsg_t *msg )
{
  ...
  /* Validate */
  if (htsmsg_get_u32(msg, "recordingId", &u32))
  {
