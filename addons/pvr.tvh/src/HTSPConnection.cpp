...
CHTSPConnection::CHTSPConnection ()
  : m_socket(NULL), m_regThread(this),
    m_ready(false), m_seq(0), m_challenge(NULL)
{
  CreateThread();
}
...
bool CHTSPConnection::ReadMessage ( void )
{
  ...
  while (cnt < len)
  {
    r = m_socket->Read(buf + cnt, len - cnt, g_iResponseTimeout);
    if (r < 0)
    {
...
bool CHTSPConnection::SendAuth ( void )
{
  htsmsg_t *msg = htsmsg_create_map();
  htsmsg_add_str(msg, "username", g_strUsername.c_str());

  /* Add Password */
  // Note: we MUST send a digest or TVH will not evaluate the
  //       message, so even with --noacl it won't work
  struct HTSSHA1* sha = (struct HTSSHA1*)malloc(hts_sha1_size);
  uint8_t d[20];
  hts_sha1_init(sha);
  hts_sha1_update(sha, (const uint8_t*)g_strPassword.c_str(),
                  g_strPassword.length());
  if (m_challenge)
...
void* CHTSPConnection::Process ( void )
{
  while (!IsStopped())
  {
    ...
    /* Create socket (ensure mutex protection) */
    {
      ...
      if (m_socket)
        delete m_socket;
      tvhdebug("connecting to %s:%d",
                g_strHostname.c_str(), g_iPortHTSP);
      m_socket = new CTcpSocket(g_strHostname, g_iPortHTSP);
      m_ready  = false;
      m_seq    = 0;
      if (m_challenge) {
        free(m_challenge);
    ...
    if (!m_socket->Open(g_iConnectTimeout * 1000))
    {
